...
namespace JanuszMarcinik.Mvc.Domain.Services.Identity.Users
{
    ...
    public class UsersService : UserManager<User, int>
    {
        public UsersService(UsersRepository repository) : base(repository)
        {
        }

        public static UsersService Create(IdentityFactoryOptions<UsersService> options, IOwinContext context)
        {
            var manager = new UsersService(new UsersRepository(context.Get<ApplicationDbContext>()));
